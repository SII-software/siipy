...
md = modeling()
dfname = '../html/html/regor/intfer/sdata.npy'
dfname1 = '../html/html/regor/intfer/mdata.npy'
md.sii_para(Area = 100, delt = 5e-9, itta = 0.30, loss = 0.50, avgt = orbint.avgt, chnl=1)
md.star_para(Tmp_a = 35000, Tmp_b = 57000, Tmp_r = 13880, dis_e = 3.458e10, lam_c = 370e-9, lam_e = 465e-9)
...
fname2 = '../html/html/regor/intfer/dres_all.pkl'
pnames = ('R_a (ls)', 'R_b (ls)', 'ld', 'x0', 'y0', 'x1', 'y1', 'x2', 'y2', 'x3', 'y3')
fname = [fname2, dfname, pnames]
p_min = [0.5*sf[0], 0.5*sf[1], 0.5*sf[3], 0.5*zs[0], 0.5*zs[1], 0.5*zs[2], 0.5*zs[3], 0.5*zs[4], 0.5*zs[5], 0.5*zs[6], 0.5*zs[7]]
p_max = [1.5*sf[0], 1.5*sf[1], 1.5*sf[3], 1.5*zs[0], 1.5*zs[1], 1.5*zs[2], 1.5*zs[3], 1.5*zs[4], 1.5*zs[5], 1.5*zs[6], 1.5*zs[7]]
para = 11
md.estimate(fname, bs, uv, sf, p_min, p_max, para, pl=pl)  # estimate the parameters


# Corner plot for each parameter
truth = [sf[0], sf[1], sf[3], zs[0], zs[1], zs[2], zs[3], zs[4], zs[5], zs[6], zs[7]]
cfile = '../html/html/regor/intfer/para_all'
